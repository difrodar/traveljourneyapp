...
Beispiele:
- **Repository & Struktur:** GitHub Repository mit SvelteKit-Standardstruktur. Wichtige Bereiche: `src/routes` für Pages, `src/lib/components` für UI, `src/lib/server` für MongoDB-Zugriff, `scripts/seed.js` für Seed-Daten und `scripts/upsert-sample-events.js` für non-destruktive Beispiel-Daten in einer bestehenden Datenbank.
