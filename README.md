...
- **Wertorientierte Anforderungen (User Stories):** Aus Personas und HMW-Fragen wurden die zentralen Workflows als testbare User Stories nach dem INVEST-Prinzip formuliert und jeweils an eine Persona und die umgesetzte Funktion geknüpft: [`docs/methods/user-stories.md`](docs/methods/user-stories.md).
...
- **Auswahlkriterien UI-Konzept (Sketch-Varianten A/B/C):** Die drei Grobkonzepte aus [§3.2](#32-sketch) wurden gewichtet bewertet (Skala 0–3 pro Kriterium, Score = Wert × Gewicht).

  | Kriterium (Gewicht) | A: Dashboard-zentriert | B: Map-zentriert | C: Journey-zentriert |
  |---|:---:|:---:|:---:|
  | Schneller Einstieg in die Planung (3) | 3 (9) | 1 (3) | 1 (3) |
  | Täglicher Arbeitsbereich für Events (3) | 2 (6) | 1 (3) | 1 (3) |
  | Erinnerungs-/Rückblickwert (2) | 1 (2) | 2 (4) | 3 (6) |
  | Räumliche Orientierung (2) | 1 (2) | 3 (6) | 1 (2) |
  | Geringe Navigationskomplexität (1) | 2 (2) | 2 (2) | 2 (2) |
  | **Gewichteter Score** | **21** | **18** | **16** |

  Variante A trägt als täglicher Einstieg am stärksten, doch Bs räumliche Orientierung und Cs Rückblickwert sind eigenständige Stärken. Deshalb wurde keine reine Variante übernommen, sondern eine **kombinierte** App: Dashboard als Einstieg (A), Events als Arbeitsbereich, Journey (C) und Map (B) als Rückblick/Orientierung — die drei Stärken werden zusammengeführt statt gegeneinander ausgespielt.
...
- **Entscheidungsmatrix Technologie & Datenhaltung:** Drei realistische Umsetzungsvarianten wurden gewichtet bewertet (Skala 0–3 pro Kriterium, Score = Wert × Gewicht).
  ...
  | Erweiterbarkeit für 12 Erweiterungen (2) | 1 (2) | 2 (4) | 3 (6) |
...
- **Reviewer-Login:** Für die Begutachtung steht ein vorbereiteter Demo-Account bereit, der alle Kern-Workflows und sämtliche Erweiterungen (§4.1–§4.12) mit frischen, sinnvoll datierten Beispieldaten zeigt — Username `dozent`, Passwort `Prototyping2026`. Der Account enthält u. a. abgeschlossene Events mit Memories, einen Trip, eine wiederkehrende Serie, eine offene Einladung sowie ein bevorstehendes Event mit aktivem In-App-Reminder.
...
- **Besondere Entscheidungen:** Der MongoDB Connection String wird nicht im Repository gespeichert. Die Map nutzt Leaflet und OpenStreetMap, damit keine Google-Maps-Lizenz oder kein API-Key notwendig ist. Die Map gruppiert Orte nach Land und Stadt und zeigt darunter die verknüpften Events, damit konkrete Locations wie Golden Gate Bridge oder Griffith Observatory klarer sind als reine Stadtmarker. Überlappende Pins werden über `leaflet.markercluster` zu einem Cluster mit Anzahl-Badge zusammengefasst, der beim Klick aufklappt; Filteränderungen passen den Kartenausschnitt automatisch an die gefilterten Events an. Im lokalen Dev-Modus (`npm run dev`) werden zur Bequemlichkeit die Prototype-Accounts `difrodar`/`difrodar` und `dummy`/`dummy` automatisch angelegt; bestehende Daten ohne `userId` werden dabei `difrodar` zugewiesen, `dummy` startet leer, und bei leerer Datenbank entstehen Demo-Daten nur für `difrodar`. Auf dem Produktiv-Deployment läuft dieses Auto-Seeding bewusst nicht — dort steht für die Begutachtung der Account `dozent` per Seed-Skript bereit (siehe Reviewer-Login oben). Bestehende MongoDB-Daten können non-destruktiv per Upsert-Script ergänzt oder präzisiert werden. Für Bilder nutzt die App einerseits echte, lizenzierte Wikimedia-Commons-Fotos als Fallback und andererseits eigene Uploads für Event-Cover und Journey-Memories. Uploads sind bewusst auf JPG, PNG, WebP oder GIF bis 2 MB pro Bild begrenzt; pro Event/Memory können bis zu 5 Bilder mit insgesamt höchstens 9 MB hinzugefügt werden, damit die Dokumente sicher unter dem MongoDB BSON-Limit von 16 MB bleiben. Falls für einen unbekannten Ort kein spezifisches Bild vorhanden ist, nutzt die App einen neutralen Travel-/Roadtrip-Fallback statt ein falsches Stadtbild. Auch Kategorie-Fallbacks wie `Sightseeing` sind bewusst neutral gehalten, damit konvertierte Travel Ideas nicht fälschlich ein San-Francisco-/Golden-Gate-Bild erhalten. Die Stadt- und Koordinatenauswahl ist als lokale `CityCombobox` umgesetzt: Sie nutzt eine kuratierte globale Städte-Liste mit leichtem USA-/San-Diego-Ranking, speichert `city`, `country`, `lat` und `lng` und verhindert falsche Fallback-Pins, indem unbekannte Orte ohne Koordinaten nicht als Kartenmarker erscheinen.
...
- **Stichprobe:** 2 Testpersonen (Lukas, Sandra). Beide ohne aktive Vorerfahrung mit Reise-, Kalender- oder Tagebuch-Apps, beide vertraut mit englischen Benutzeroberflächen. Die bewusst kleine Stichprobe (n=2) ist eine Scope-Entscheidung des Einzelprojekts: Der Fokus liegt auf qualitativen Reibungen, Begriffskonfusion und Umwegen statt auf statistischer Repräsentativität; der Analyse-Report empfiehlt für die nächste Iteration selbst 3–5 weitere Tests. Ausgefüllte Moderationsbögen: [Moderationsbogen Lukas](docs/validate/Usability-Test/Moderationsbogen/Moderationsbogen_Testbenutzer_Lukas.pdf), [Moderationsbogen Sandra](docs/validate/Usability-Test/Moderationsbogen/Moderationsbogen_Testbenutzer_Sandra.pdf).
...
Dieser Abschnitt dokumentiert Repository-Struktur, NPM-Scripts, Issue-Management, Commit- und Branch-Praxis sowie die zusätzlich erstellten methodischen Artefakte.
...
- **Issue-Management:** Zwei Phasen sind im Repo sichtbar.
  ...
  - **Hinweis zur Issue-Sichtbarkeit:** Alle Plan- und Usability-Issues (#1–#48) sind inzwischen **geschlossen**; der GitHub-Issues-Tab zeigt deshalb standardmässig „0 open". Die vollständige Arbeitshistorie bleibt über den Label-Filter [`usability-test`](https://github.com/difrodar/traveljourneyapp/issues?q=label%3Ausability-test) sowie den [Filter für geschlossene Issues](https://github.com/difrodar/traveljourneyapp/issues?q=is%3Aissue+is%3Aclosed) einsehbar.
