...
- **Image uploads:** 2 MB max; JPG/PNG/WebP/GIF only. Server-side check in [shared.js](src/lib/server/repositories/shared.js) is the source of truth; client guards in [EventForm.svelte](src/lib/components/EventForm.svelte) and [MemoryForm.svelte](src/lib/components/MemoryForm.svelte).
